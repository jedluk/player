...
export const formatTime = (fromDate: Date): string => {
  ...
  return `${pad(`${date.getMonth() + 1}`, 2)}.${pad(
    `${date.getDate()}`,
    2
  )}.${date.getFullYear()}`
}
...
export function isValidTrack(
  ...
  return (
    (file as API.Track).album !== undefined &&
    (file as API.Track).title !== undefined
  )
}

export function isValidDirectory(
  file: API.Track | API.Directory
): file is API.Directory {
  return (
    (file as API.Directory).url !== undefined &&
    (file as API.Directory).name !== undefined
  )
}
